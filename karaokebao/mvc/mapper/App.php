...
class App extends Mapper implements \MVC\Domain\AppFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from karaokebao_app");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from karaokebao_app where id=?");
		$this->findByStmt = self::$PDO->prepare( 
                            "select * from karaokebao_app where alias=?");
        ...
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into karaokebao_app ( name, phone, address, email, banner, prefix, app, datecreate, dateupdate, dateactivity, type) values(?,?,?,?,?,?,?,?,?,?,?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from karaokebao_app where id=?");
		$this->checkStmt = self::$PDO->prepare( 
                            "select distinct id from karaokebao_app where app=?");			
		$this->checkEmailStmt = self::$PDO->prepare( 
                            "select distinct id from karaokebao_app where email=?");									
