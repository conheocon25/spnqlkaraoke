...
class OrderImportDetail extends Mapper implements \MVC\Domain\OrderImportDetailFinder {
    ...
    function __construct() {
		...
		$tblResource = "karaokebao_resource";
		$tblOrderImport = "karaokebao_order_import";
		$tblOrderImportDetail = "karaokebao_order_import_detail";
		$tblSessionDetail = "karaokebao_session_detail";
		$tblR2C = "karaokebao_r2c";
		...
		$trackByExportStmt = sprintf("
			...
			from
				karaokebao_session S inner join karaokebao_session_detail SD on S.id = SD.idsession
			where
				SD.idcourse IN(select id_course from karaokebao_r2c where id_resource=?) AND
