...
class Customer extends Mapper implements \MVC\Domain\CustomerFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from karaokebao_customer");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from karaokebao_customer where id=?");
        $this->updateStmt = self::$PDO->prepare( 
                            "update karaokebao_customer set name=?, type=?, card=?, phone=?, address=?, note=?, discount=? where id=?");
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into karaokebao_customer (name, type, card, phone, address, note, discount) 
							values( ?, ?, ?, ?, ?, ?, ?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from karaokebao_customer where id=?");
		$this->findByPositionStmt = self::$PDO->prepare("
						SELECT id 
						FROM karaokebao_customer
						WHERE idlocation=?
						LIMIT ?,1
						ORDER By id asc
		");
		$this->findByCardStmt = self::$PDO->prepare("select * from karaokebao_customer where card=?");
		
		$tblCustomer = "karaokebao_customer";
	...
	function findByPostion($values) {		
        $str = "SELECT id FROM karaokebao_customer ORDER BY id LIMIT ". $values[0] .",1";		
