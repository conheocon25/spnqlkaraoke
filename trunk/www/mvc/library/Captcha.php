...
Class Captcha {
	...
	function __construct($width = 150, $height = 80) {      
		$this->width = $width;
		$this->height = $height;
    }
	...
	{ 		
		...
		$greyNew = ImageColorAllocate($image, 36, 43, 53); 
		
		
		
		//Make the background black 
		//ImageFill($image, 0, 0, 709, 150, $white); 	
		imagefilledrectangle($image, 0,0, 709, 99, $greyNew);
		//Add randomly generated string in white to the image
		ImageString($image, 5 ,5, 5, $this->SecurityCode, $red); 
		//imagestring ($im, $font, $x, $y, $string, $textColor);
		//Throw in some lines to make it a little bit harder for any bots to break 
		ImageRectangle($image,0,0, $this->width-1, $this->height-1, $greyNew); 
